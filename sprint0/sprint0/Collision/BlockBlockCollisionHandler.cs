...
namespace sprint0
{
    public class BlockBlockCollisionHandler
    {
        ...
        public void HandleCollision(IBlock block1, IBlock block2, Direction side)
        {
            ...
            else if (block2.IsMovable(side)) HandleMovableImovableBlock(block1, block2, side);
            else if (block1.IsMovable(side)) HandleImmovableBlock(block1, block2, side);
        ...
        private void HandleMovableImovableBlock(IBlock block1, IBlock block2, Direction side)
        {
            switch (side)
            {
                case Direction.North:
